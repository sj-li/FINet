...
def visualize_scenario(
    ...
    cur_plot_bounds, focal_hist_traj, focal_fut_traj = _plot_actor_tracks(ax, scenario, timestep)
    plot_bounds = cur_plot_bounds
    # Assuming data is a dictionary containing 'x_positions' and 'x_angles'
    ...
    rotate_mat_inv = rotate_mat.T
    prediction  = (prediction.astype(np.float64)).dot(rotate_mat_inv) + origin.astype(np.float32)
    ...
    # import pdb
    # pdb.set_trace()
    # prediction = prediction + origin[None,:,:]


    # # Assuming pos and valid_mask are NumPy arrays with appropriate shapes
    # prediction = (prediction.astype(np.float64) - origin).dot(rotate_mat).astype(np.float32).squeeze(-1)  # [13, 90, 2]
...
def _plot_actor_tracks(
    ...
    for track in scenario.tracks:
        ...
        if track.category == TrackCategory.FOCAL_TRACK:
            ...
            focal_hist_traj = history_trajectory
            focal_fut_traj = future_trajectory
    ...
    return track_bounds, focal_hist_traj, focal_fut_traj
